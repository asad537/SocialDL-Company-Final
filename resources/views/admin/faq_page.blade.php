...
@section('content')
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <div><h3>FAQ Page SEO Settings</h3><p>Manage metadata for the FAQ page</p></div>
        </div>
        <form method="POST" action="{{ route('admin.faq_page.seo.save') }}">
            @csrf
            <div class="form-group">
                <label>Meta Title</label>
                <input type="text" name="faq_meta_title" value="{{ $settings->faq_meta_title ?? '' }}">
            </div>
            <div style="display:grid; grid-template-columns:1fr 1fr; gap:1.2rem;">
                <div class="form-group">
                    <label>Meta Description</label>
                    <textarea name="faq_meta_description" rows="3">{{ $settings->faq_meta_description ?? '' }}</textarea>
                </div>
                <div class="form-group">
                    <label>Meta Keywords</label>
                    <textarea name="faq_meta_keywords" rows="3">{{ $settings->faq_meta_keywords ?? '' }}</textarea>
                </div>
            </div>
    ...
    <div class="form-card">
        <div class="form-card-header">
            ...
            <div><h3>Add New FAQ to Page</h3><p>Categorized help content</p></div>
        </div>
    ...
    <div class="form-card">
        ...
        @empty
            <p style="text-align:center;padding:2rem;color:rgba(255,255,255,0.2);">No FAQs yet.</p>
        @endforelse
    </div>
@endsection
