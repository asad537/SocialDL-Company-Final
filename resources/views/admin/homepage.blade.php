...
@push('styles')
    ...
    <style>
        ...
        #hero_description_editor::-webkit-scrollbar { width: 6px; }
        #hero_description_editor::-webkit-scrollbar-track { background: #f1f5f9; border-radius: 0 12px 12px 0; }
        #hero_description_editor::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 10px; }
        #hero_description_editor::-webkit-scrollbar-thumb:hover { background: #94a3b8; }
    ...
    <form id="homepageForm" method="POST" action="{{ route('admin.homepage.save') }}">
        ...
        <div class="form-card">
            ...
            <div class="form-row full">
                <div class="form-group">
                    ...
                    <textarea name="sites_description" rows="3" placeholder="Short description...">{{ optional($settings)->sites_description ?? '' }}</textarea>
                </div>
            </div>
        ...
        <div class="preview-section">
            ...
            <div class="preview-mockup">
                ...
                <a class="preview-btn-el" href="#">{{ optional($settings)->hero_button_text ?? 'Button Preview' }}</a>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', async () => {
            ...
            try {
                ...
                if (rawContent) {
                    try {
                        // Check if it's already an object with blocks (from cast or JSON)
                        if (typeof rawContent === 'object' && rawContent !== null && rawContent.blocks) {
                            ...
                            try {
                                ...
                                if (parsed && typeof parsed === 'object' && parsed.blocks) {
                                    ...
                                    throw new Error('Not valid EditorJS JSON');
                                }
                            } catch (parseError) {
                                // Fallback for strings
                                if (rawContent.includes('<') || rawContent.includes('>')) {
                                    console.log('Treating content as HTML');
                        ...
                        console.error('Error processing initialData:', e);
                    }
                }

                console.log('Final initialData for Editor:', initialData);

                heroEditor = await editorJSManager.init('hero_description_editor', {
                    placeholder: 'Write something amazing about your service...',
                    minHeight: 300,
                    data: initialData
                });
