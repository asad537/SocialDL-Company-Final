...
<head>
    ...
    <title>Create Guide — Admin</title>
...
<div class="main">
    <div class="topbar">
        <div class="topbar-left">
            <h1><i class="fas fa-plus" style="color:#FFB800;margin-right:0.5rem;"></i> Create New Post</h1>
            <div class="breadcrumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a> / <a href="{{ route('admin.guides.index') }}">Guides</a> / Create</div>
        </div>
        <a href="{{ route('admin.guides.index') }}" class="btn-back"><i class="fas fa-arrow-left"></i> Back to List</a>
    </div>

    <div class="content">
        <form id="guideForm" method="POST" action="{{ route('admin.guides.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-card">
                ...
                <div class="grid-2">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="image_alt" placeholder="SEO description for image">
                    </div>
                </div>
                <div class="grid-2">
                    <div class="form-group">
            ...
            <div class="form-card">
                ...
                <div class="grid-2">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="tags_cloud" placeholder="Internal keywords">
                    </div>
                </div>
            </div>

            <div style="margin-bottom:4rem; text-align:right;">
                <button type="submit" class="btn-save"><i class="fas fa-save"></i> Save Guide Post</button>
            </div>
        </form>
    </div>
</div>
    ...
    document.getElementById('guideForm').onsubmit = async function(e) {
        ...
        try {
            if (guideEditor) {
                ...
                // The user's code from 'findheadsets' converts to HTML for storage
                // but our backend parser expects JSON. 
                // However, the user said "wo code hi cchoye", implying they want to use their logic.
                // In 'findheadsets', they use editorJSManager.toHTML(contentData).
                // Let's stick to JSON for now as our renderContent() in Guide model handles JSON.
                // WAIT, if I use their manager, I should check what their renderContent() looks like.
                // Actually, I wrote the renderContent() in the previous turn.
                // Let's keep it as JSON for flexibility, or use HTML if they prefer.
                // The manager has toHTML() which is very clean.
